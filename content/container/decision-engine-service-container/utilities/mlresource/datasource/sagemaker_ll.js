...
const registerTrainingProcess = require('../model/sagemaker_ll');
...
async function uploadDatasource({ rows, data_source_name, }) {
  try {
    ...
    let Key = `datasources/sagemaker_ll/${original_filename}`;
...
async function transformLinearLearner({ mlmodel, headers, training_data_transposed, testing_data_transposed, columnTypes, }) {
  try{
    ...
    await Datasource.update({
      ...
      isPatch: true,
      updatedoc: {
        'providers.sagemaker_ll.headers': options.headers,
        'providers.sagemaker_ll.training': options.training,
        'providers.sagemaker_ll.testing': options.testing,
        'providers.sagemaker_ll.trainingbatch': options.trainingbatch,
      },
      updatedat: new Date(),
    });
...
module.exports = transformLinearLearner;