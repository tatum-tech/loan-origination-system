...
describe('Valid Periodic extension', function () {
  ...
  describe('complete extension folder structure', function () {
    ...
    it('should export default settings config', () => {
      const SETTINGS_config = require(path.join(extension_root_dir, 'config/settings'));
      expect(SETTINGS_config).to.be.an('object');
    });
    ...
    it('should export valid functions from transforms', () => {
      if (extension_files.indexOf('transforms') > -1) {
        ...
        Object.keys(flattened_transforms).forEach(transform => {
          if (flattened_transforms[ transform ] && Object.keys(flattened_transforms[ transform ]).length) {
